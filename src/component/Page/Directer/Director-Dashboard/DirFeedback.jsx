...
const Icon = ({ name, className = "w-5 h-5", fillColor = "currentColor" }) => {
  let path = "";
  switch (name) {
    case "FaStar":
  ...
  return (
    <svg className={className} fill={fillColor} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <path d={path} />
    </svg>
  );
};
...
const DirFeedback = () => {
  ...
  const starScale = {
    1: { label: 'Very Bad', tooltip: 'Extremely dissatisfied', color: 'bg-red-100 text-red-800' },
    2: { label: 'Bad', tooltip: 'Somewhat dissatisfied', color: 'bg-orange-100 text-orange-800' },
    3: { label: 'Neutral', tooltip: 'Neither satisfied nor dissatisfied', color: 'bg-yellow-100 text-yellow-800' },
    4: { label: 'Good', tooltip: 'Somewhat satisfied', color: 'bg-blue-100 text-blue-800' },
    5: { label: 'Excellent', tooltip: 'Extremely satisfied', color: 'bg-green-100 text-green-800' },
  };
      ...
      <div className="container mx-auto px-0 sm:px-4 max-w-full h-screen flex flex-col">
        ...
        <div className="relative bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 py-6 sm:px-8 sm:py-8 text-center">
          ...
          <div className="relative z-10">
            <h1 className="text-xl sm:text-2xl font-bold mb-2">Director Feedback</h1>
            <p className="text-indigo-100 mb-3 text-sm sm:text-base">
              Your insights help shape our future
            </p>
            {formData.rating > 0 && (
              <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${getRatingColor(formData.rating)}`}>
                ...
                <span className="ml-1">{starScale[formData.rating].label}</span>
              </div>
            )}
          </div>
        </div>
              ...
              <>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    name="feedback"
                    placeholder="What are your insights? How can we improve?"
                      ...
                      <div key={value} className="flex flex-col items-center group relative">
                        <button
                          ...
                          className={`text-3xl sm:text-4xl cursor-pointer transition-all duration-200 transform ${
                            value <= (uiState.hoverRating || formData.rating) 
                          ...
                          onMouseEnter={() => setUiState(prev => ({ ...prev, hoverRating: value }))}
                          onMouseLeave={() => setUiState(prev => ({ ...prev, hoverRating: 0 }))}
                          aria-label={`Rate ${value} star${value === 1 ? '' : 's'}`}
                        >
                          <Icon name="FaStar" fillColor="currentColor" />
                        ...
                        <div className={`text-xs mt-1 ${
                          ...
                          {uiState.isMobile ? '' : starScale[value].label}
                        </div>
                        {formData.rating === value && (
                          <div className={`absolute -bottom-2 ${starScale[value].color} text-xs px-2 py-0.5 rounded-full`}>
