...
function DirectorNavbar({ sidebarOpen, setSidebarOpen, setActiveTab }) {
  ...
  const lastLogin = "2024-07-11 16:00 IST"; // Example data
  const memberSince = "2020-05-15"; // Example data

  // Pages relevant to a Director
  const pages = [
    { key: "reports", label: "📊 Reports", sectionId: "reports-section" },
    { key: "requests", label: "📋 Requests Overview", sectionId: "requests-section" }, // Director might see an overview of requests
    { key: "userManagement", label: "👥 User Management", sectionId: "userManagement-section" },
    { key: "systemSettings", label: "⚙️ System Settings", sectionId: "systemSettings-section" },
    { key: "activityHistory", label: "📜 Activity History", sectionId: "activityHistory-section" },
    { key: "feedback", label: "💬 Feedback Analysis", sectionId: "feedback-section" },
    { key: "knowledgeBase", label: "📚 Knowledge Base", sectionId: "knowledgeBase-section" },
    { key: "contactSupport", label: "📞 Contact Log", sectionId: "contactSupport-section" },
  ];
  ...
  // Profile Menu Action Handlers (similar to ITNavbar, but for Director)
    ...
    setModalContent({
      ...
      content: (
        <>
          ...
          <p><strong>Email:</strong> {directorEmail}</p>
          <p><strong>Role:</strong> {directorRole}</p>
          <p><strong>Last Login:</strong> {lastLogin}</p>
          <p><strong>Member Since:</strong> {memberSince}</p>
          <p className="mt-4 text-sm text-gray-500">
            (In a real application, this would be a dedicated profile page for the Director.)
          </p>
        </>
      ),
    });
  ...
  const handleAccountSettings = () => {
    setModalContent({
      ...
      content: (
        <>
          <p>Here you can update your password, change your display name, or manage security preferences specific to your Director account.</p>
          <ul className="list-disc list-inside mt-2 text-sm text-gray-600">
            <li>Change Password</li>
            <li>Update Email Address</li>
            <li>Two-Factor Authentication</li>
            <li>Manage API Keys (if applicable)</li>
          </ul>
          <p className="mt-4 text-sm text-gray-500">
  ...
  const handleNotificationSettings = () => {
    setModalContent({
      ...
      content: (
        <>
          <p>Manage how you receive notifications for critical reports, system alerts, and key performance indicators.</p>
          <ul className="list-disc list-inside mt-2 text-sm text-gray-600">
            <li>Email Notifications: <span className="font-semibold">On</span></li>
            <li>Executive Summary Emails: <span className="font-semibold">Daily</span></li>
            <li>Critical Alert SMS: <span className="font-semibold">On</span></li>
            <li>Dashboard Push Notifications: <span className="font-semibold">On</span></li>
          </ul>
          <p className="mt-4 text-sm text-gray-500">
  ...
  const handleSignOut = () => {
    console.log("Signing out Director and redirecting to /login/director...");
    // In a real app, you would perform actual logout logic
        ...
        >
          ...
          <div className="hidden md:flex items-center">
            <div className="text-right mr-1">
              <p className="text-sm font-medium text-gray-800">{directorName}</p>
              <p className="text-xs text-gray-500">{directorRole}</p>
            </div>
          ...
          {showProfileMenu && (
            <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-100 py-2 animate-fadeIn z-50">
              <div className="px-4 py-3 border-b border-gray-100">
                <p className="text-sm font-medium text-gray-800">{directorName}</p>
                <p className="text-xs text-gray-500 truncate">{directorEmail}</p>
              </div>
              <div className="py-1">
                <button
