...
function DirectorLogin() {
  ...
  const [form, setForm] = useState({
    ...
    email: "",
    password: "",
    directorRole: "Select Director Role",
    captchaInput: "",
  });
  ...
  const handleLogin = (e) => {
    ...
    setTimeout(() => {
      navigate("/director-home"); // Navigate to director dashboard after a short delay
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-indigo-100 to-blue-200 px-4">
      ...
      <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md space-y-6">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          {/* Director Role Select */}
          <div>
            <label htmlFor="directorRole" className="block text-sm font-semibold text-gray-700 mb-1">
              Director Role
            </label>
            <select
              id="directorRole"
              ...
              className="w-full p-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none transition duration-150"
            >
              <option value="Select Director Role">Select Director Role</option>
              <option>Head of Director's</option>
              <option>Group Director</option>
              <option>Director Secretory</option>
            </select>
          </div>
          ...
          <div className="text-center mt-4 text-sm">
            ...
            <Link to="/register-Directer" className="text-blue-600 font-medium hover:underline">
              Register
            </Link>{" "}
            |{" "}
            <Link to="/forgot-password-Directer" className="text-blue-600 font-medium hover:underline">
              Forgot Password?
            </Link>
          </div>
        </form>
